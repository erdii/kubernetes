...
func ValidatePatchOptions(options *metav1.PatchOptions, patchType types.PatchType) field.ErrorList {
	...
	case types.ApplyYAMLPatchType, types.ApplyCBORPatchType:
		...
		// Validate mutual exclusivity of CreateOnly and UpdateOnly.
		if options.CreateOnly != nil && *options.CreateOnly && options.UpdateOnly != nil && *options.UpdateOnly {
			allErrs = append(allErrs, field.Invalid(field.NewPath("createOnly"), *options.CreateOnly, "may not be true when updateOnly is true"))
		}
	default:
		...
		if options.UpdateOnly != nil {
			allErrs = append(allErrs, field.Forbidden(field.NewPath("updateOnly"), "may not be specified for non-apply patch"))
		}
	}
	allErrs = append(allErrs, ValidateFieldManager(options.FieldManager, field.NewPath("fieldManager"))...)
	allErrs = append(allErrs, ValidateDryRun(field.NewPath("dryRun"), options.DryRun)...)
